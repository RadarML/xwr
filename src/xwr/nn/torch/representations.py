...
class Representation(ABC):
    ...
    @abstractmethod
    def __call__(
        self, spectrum: Complex64[Tensor, "batch doppler el az rng"],
        aug: Mapping[str, Any] = {}
    ) -> Float32[Tensor, "batch doppler el az rng c"]:
...
class Magnitude(Representation):
    ...
    def __call__(
        self, spectrum: Complex64[Tensor, "batch doppler el az rng"],
        aug: Mapping[str, Any] = {}
    ) -> Float32[Tensor, "batch doppler el az rng 1"]:
        ...
        if aug.get("azimuth_flip", False):
            spectrum = torch.flip(spectrum, dims=(-2,))
        if aug.get("doppler_flip", False):
            spectrum = torch.flip(spectrum, dims=(-3,))
...
class PhaseAngle(Representation):
    ...
    def __call__(
        self, spectrum: Complex64[Tensor, "batch doppler el az rng"],
        aug: Mapping[str, Any] = {}
    ) -> Float32[Tensor, "batch doppler el az rng 2"]:
        ...
        return torch.stack([
            ...
            resize(phase, range_scale, speed_scale) % (2 * torch.pi)
        ], dim=-1)
...
class PhaseVec(Representation):
    ...
    def __call__(
        self, spectrum: Complex64[Tensor, "batch doppler el az rng"],
        aug: Mapping[str, Any] = {}
    ) -> Float32[Tensor, "batch doppler el az rng 3"]:
        ...
        magnitude = torch.abs(spectrum)
        normed = spectrum / torch.maximum(magnitude, torch.Tensor(self.eps))
        if aug.get("radar_phase", 0.0) != 0.0:
            normed *= torch.exp(-1j * aug["radar_phase"])
        re = torch.real(normed)
        im = torch.imag(normed)
